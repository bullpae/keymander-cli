...
| **R1-2. spawn+토글 레이스 제거** | ⬜ 데몬 Launch를 "떠 있으면 표시 신호, 없으면 spawn"으로. 콜드스타트 중 재-핫키가 토글오프시키는 문제의 근본 |
| **R1-3. 콜드 스타트 실측·개선** | ⬜ 하루 첫 실행 지연. 측정 먼저 — 용의자: GPU 어댑터 프로빙(코드 주석상 수 초), 인덱스 캐시 6h staleness, dyld 18MB |
