...
1. **TUI의 config 두 벌 정리** — R2-7에서 캐시를 넣었지만 메인 루프 소유
   `config`와 `AppState.config`가 공존한다. 저장 시 함께 갱신하고 있어 지금은
   맞지만, 한쪽만 고치는 실수가 언제든 가능하다
2. **R2-2 `kmd-desktop/app.rs` 분해 계속** — `view_detail_panel`·`update_inner`가
