...
> **상태: 구현 완료 (v0.10.0, Windows 전용).** 이 문서는 **설계 이력**이다.
> 구현은 `crates/kmd-daemon/src/autopilot.rs` — 키보드 훅 엔진과 분리된 독립
> 모듈이고, Windows 외 플랫폼은 스텁이라 프런트엔드가 URL 폴백을 쓴다.
> 안전 설계(타이틀 게이트)의 근거가 이 문서에 있으니 코드를 고치기 전에 읽을 것.
