...
| 북마크 | 자주 쓰는 항목 고정 |
| 인라인 계산기 | `:calc` 접두사 또는 수식 자동 감지 — 계산 결과 표시, 클립보드 복사 |
| 단일 인스턴스 토글 | 핫키 재입력 시 기존 인스턴스 종료 (on/off 토글) |
| 포터블 모드 | `kmd portable enable` — exe 옆 kmd-data/ 사용 |
| 윈도우 센터링 | Windows 핫키 실행 시 콘솔 창 화면 중앙 배치 |
