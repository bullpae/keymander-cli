...
- **2단계 엔진 로딩**: 시작 시 `quick` 엔진(앱/PATH/시스템 명령)으로 즉시 검색 가능 상태를 만들고, 백그라운드에서 전체 파일 인덱스를 로드한 뒤 hot-swap
- **기본 인덱싱 경량화**: 기본값을 `max_results=5000`, `search_depth=4`, `scan_drives=false`, `drive_scan_depth=2`로 조정
- **타이밍 로깅 추가**: quick/full 엔진 준비 시간, 캐시 hit/build 시간을 로그로 확인 가능
