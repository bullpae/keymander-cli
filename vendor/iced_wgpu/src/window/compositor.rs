...
impl Compositor {
    ...
    pub async fn request<W: compositor::Window>(
        ...
        // [keymander patch] 백엔드 옵션을 환경변수에서 읽는다.
        //
        // 업스트림은 `..Default::default()` 라서 DX12 프레젠테이션 방식이
        // `DxgiFromHwnd` 로 고정된다. HWND 스왑체인은 per-pixel 알파를
        // 지원하지 않아(alpha_modes = [Opaque]) Windows 에서 투명 창이
        // 불가능하다. `WGPU_DX12_PRESENTATION_SYSTEM=visual` 을 읽게 하면
        // DirectComposition 경로가 열려 PreMultiplied 를 쓸 수 있다.
